...
- **회피(Dodge) 시스템**: 스페이스바 입력 시 지정된 방향으로 순간 이동하며, 이동 중에는 적과의 충돌을 무시해 일시적인 무적 상태가 됨
...
Assets/
 └─ Scripts/
     ├─ PlayerController.cs   # 플레이어 이동, 조준, 사격, 회피
...
혹은 https://sywoo0109.itch.io/top-down-shooter-toy-project 에서 플레이 가능합니다

## 📝 제작 배경

Unity 기초 강좌를 참고해 처음 만든 토이 프로젝트를 바탕으로, 실무에서 흔히 쓰이는 구조와 패턴(캐싱, 이벤트 기반 설계, 신버전 Input System 등)을 스스로 학습하며 리팩토링하고, 회피 기능 등을 새로 추가하며 발전시킨 프로젝트입니다.
